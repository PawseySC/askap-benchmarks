...
// This is replaced due to lack of HIPCPU support
//    grid[gind+threadIdx.x] = hipCfmaf(l_data, C[cind+threadIdx.x], grid[gind+threadIdx.x]);

    grid[gind+threadIdx.x].x = l_data.x * C[cind+threadIdx.x].x - l_data.y * C[cind+threadIdx.x].y + grid[gind+threadIdx.x].x;
    grid[gind+threadIdx.x].y = l_data.y * C[cind+threadIdx.x].x + l_data.x * C[cind+threadIdx.x].y + grid[gind+threadIdx.x].y;
